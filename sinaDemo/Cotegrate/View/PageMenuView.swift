...
class PageMenuView: UIView {
    ...
    fileprivate lazy var scrollLine :UIView = {
        ...
        scrollViewLine.backgroundColor = selectedColor
...
extension PageMenuView{
    ...
    fileprivate func setupTilteButton() {
    ...
    fileprivate func setupButtonLine(){
        ...
        guard let firstButton = titleButtons.first else {
            return
        }
...
extension PageMenuView {
    ...
    @objc fileprivate func titleButtonClick(sender : UIButton){
       setupTitleCenter(button: sender)
        if sender.tag == currentButtonIndex {
            return
        }
    ...
    // MARK: - 点击添加按钮  在之前的两个按钮基础上添加后面一页选中的按钮
...
extension PageMenuView {
    func setTitleWithProgress(_ progress: CGFloat,sourceIndex : Int ,targetIndex :Int) {
        ...
        sourceButton.setTitleColor(selectedColor, for: .selected)
        sourceButton.setImage(UIImage(named:titleToSting(title: sourceButton.currentTitle!) + "_h_icon_13x13_"), for: .normal)
